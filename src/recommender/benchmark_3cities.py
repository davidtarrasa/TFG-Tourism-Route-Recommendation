...
def _write_markdown(summary_path_md: str, summary: Dict[str, object]) -> None:
    ...
    for city in cities if isinstance(cities, list) else []:
        ...
        if isinstance(rank, dict) and rank:
            lines.append("### Ranking (POI + Categoría)")
            lines.append("")
            lines.append("| mode | hit | precision | recall | ndcg | cat_hit | cat_precision | cat_recall | cat_ndcg | novelty | diversity |")
            lines.append("|---|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|")
            for mode, vals in rank.items():
                if not isinstance(vals, dict):
                    continue
                lines.append(
                    f"| {mode} | {vals.get('hit', 0.0):.6f} | {vals.get('precision', 0.0):.6f} | {vals.get('recall', 0.0):.6f} | {vals.get('ndcg', 0.0):.6f} | {vals.get('cat_hit', 0.0):.6f} | {vals.get('cat_precision', 0.0):.6f} | {vals.get('cat_recall', 0.0):.6f} | {vals.get('cat_ndcg', 0.0):.6f} | {vals.get('novelty', 0.0):.6f} | {vals.get('diversity', 0.0):.6f} |"
                )
            lines.append("")
